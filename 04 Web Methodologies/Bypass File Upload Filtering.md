...
- Disable JS (may break the whole website)
- Intercept the page with Burp and remove the filter code
- Upload the file in an accepted format, intercept the upload with Burp and change the format to the desired format.
- Send the file directly to the upload point with curl.  `curl -X POST -F "submit:<value>" -F "<file-parameter>:@<path-to-file>" <site>`

## Server Side Filtering
- File extension filtering. Some servers block .php file extension, so you can try others:
`.php3`, `.php4`, `.php5`, `.php7`, `.phps`, `.php-s`, `.pht` and `.phar`

- If none of these work you can try using `.jpg.php` - some applications only check for the existence of the `.jpg`.


## Full THM Methodology
